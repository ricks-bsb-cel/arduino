...
static inline const char* wifiSecToStr(uint8_t t) {
  switch (t) {
...
void enterConfigMode() {
  ...
  DEBUG_PRINT("Ready for connection");
  DEBUG_PRINT(getWiFiName().c_str());
  DEBUG_PRINT(String("URL: ") + CONFIG_AP_URL);
#endif
  ...
  server.on("/config", []() {
    ...
    DEBUG_PRINT(String("WiFi: ") + ssid);
    DEBUG_PRINT(String("Cloud: ") + token + " @ " + host + ":" + port);
...
void enterConnectNet() {
  ...
  DEBUG_PRINT(String("WiFi: ") + configStore.wifiSSID);
  ...
  if (WiFi.status() == WL_CONNECTED) {
    DEBUG_PRINT(String("WiFi Connected!"));
